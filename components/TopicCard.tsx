...
export function TopicCard({ topic, variant = "full" }: TopicCardProps) {
  ...
  return (
    <Link
      ...
      prefetch={true}
      onClick={() => playPopSound()}
      className="group flex w-full touch-manipulation flex-col rounded-2xl border-b-4 border-slate-200 bg-white p-4 shadow-md transition-all hover:-translate-y-1 hover:rotate-1 hover:border-sky-300 hover:shadow-lg focus:ring-2 focus:ring-sky-500 focus:outline-none active:translate-y-0 active:rotate-0 sm:p-6"
    >
