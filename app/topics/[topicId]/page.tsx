...
export default async function TopicDetailPage({ params }: PageProps) {
  ...
  // Show a sample of items (up to 8)
  const sampleItems = topic.items.slice(0, 8);
            ...
            <div className="flex flex-wrap gap-3">
              {sampleItems.map((item) =>
                isColorItem(item) ? (
                  <span
                    key={item.color}
                    className="h-12 w-12 cursor-default rounded-xl border border-slate-200 shadow-sm transition-transform hover:scale-110 hover:shadow-md"
                  ...
                  <span
                    key={item.emoji}
                    className="cursor-default rounded-xl border border-sky-100 bg-gradient-to-br from-sky-50 to-blue-50 p-3 text-3xl shadow-sm transition-transform select-none hover:scale-110 hover:shadow-md"
                    title={item.word}
                  >
                    {item.emoji}
                  </span>
